...
class EmployeeList extends Component {
  ...
  componentDidMount() {
    const { payroll, account, web3 } = this.props;
    this.reload();
    const updateEmployeeList = (error, result) => {
      if (!error) {
        this.reload();
      }
    }

    this.addEmployeeEvent = payroll.AddEmployee(updateEmployeeList);
    this.upateEmployeeEvent = payroll.UpdateEmployee(updateEmployeeList);
    this.removeEmployeeEvent = payroll.RemoveEmployee(updateEmployeeList);
  }

  componentWillUnmount() {
    this.addEmployeeEvent.stopWatching();
    this.upateEmployeeEvent.stopWatching();
    this.removeEmployeeEven.stopWatching();
  }
  ...
  removeEmployee = (employeeId) => {
    ...
    payroll.removeEmployee(employeeId, {
      from: account,
      gas: 1000000,
    }).then(result => {console.log(result); alert('success');});
  }
