...
contract('Payroll', (accounts) => {
  ...
  const employeeNoExit = accounts[3];
  const salary = 1;
  ...
  it("Test call removeEmployee() to remove no exit employee", () => {
    // Remove no exit employee
    return payroll.removeEmployee(employeeNoExit, {from: owner}).then(() => {
      assert(false, "Should not be successful");
    }).catch(error => {
      assert.include(error.toString(), "Error: VM Exception", "Cannot call removeEmployee() to remove no exit employee");
    });
  });
  ...
  it("Test call removeEmployee() by employee self", () => {
    return payroll.removeEmployee(employee, {from: employee}).then(() => {
      assert(false, "Should not be successful");
    }).catch(error => {
      assert.include(error.toString(), "Error: VM Exception", "Cannot call removeEmployee() by employee self");
    });
  });
