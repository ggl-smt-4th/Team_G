...
contract('Payroll', function (accounts) {
  ...
  const salary = 1;
  const salaryOverflow = 0x123456789012345678901234123456789012345678901234123456789012345678901234;
  ...
  it("Test addEmployee() by exit employee", function () {
    var payroll;
    return Payroll.new().then(instance => {
      payroll = instance;
      return payroll.addEmployee(employee, salary, {from: owner});
      ...
      assert.include(error.toString(), "Error: VM Exception", "can not add a exit employee again!");
    });
  });

  it("Test call addEmployee() with big salary so that overflow", function () {
    var payroll;
    return Payroll.new().then(instance => {
      payroll = instance;
      return payroll.addEmployee(employee, salaryOverflow, {from: owner});
    }).then(assert.fail).catch(error => {
      assert.include(error.toString(), "RangeError:", "salary out of length");
    });
  });
});
